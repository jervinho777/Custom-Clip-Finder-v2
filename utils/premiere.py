...
Generates FCP XML files that can be imported into Premiere Pro.
Based on working V1 implementation, cleaned up for V2.
"""
...
def get_video_info(video_path: str | Path) -> Dict:
    ...
    Returns:
        Dict with duration, fps, dimensions, sample_rate, path, filename
    """
...
def generate_premiere_xml(
    ...
    """
    Generate Premiere Pro compatible XML from segments.
    
    Args:
        segments: List of dicts with 'start' and 'end' timestamps
        source_video_path: Path to source video
        output_path: Where to save the XML
        sequence_name: Name for the sequence
        markers: Optional list of markers with 'time', 'name', 'color'
    ...
    for i, seg in enumerate(segments):
        ...
        # Audio clips
        for track_idx, clips_list, cid in [(1, a1_clips, cid_a1), (2, a2_clips, cid_a2)]:
            audio_clip = f'''
				<clipitem id="clipitem-{cid}" premiereChannelType="stereo">
					<masterclipid>masterclip-1</masterclipid>
					<name>{vi['filename']}</name>
					<enabled>TRUE</enabled>
					<duration>{source_dur}</duration>
					...
					<pproTicksOut>{ticks_out}</pproTicksOut>
					<file id="file-1"/>
					<sourcetrack><mediatype>audio</mediatype><trackindex>{track_idx}</trackindex></sourcetrack>
					<link><linkclipref>clipitem-{cid_v}</linkclipref><mediatype>video</mediatype><trackindex>1</trackindex><clipindex>{i+1}</clipindex></link>
					<link><linkclipref>clipitem-{cid_a1}</linkclipref><mediatype>audio</mediatype><trackindex>1</trackindex><clipindex>{i+1}</clipindex><groupindex>1</groupindex></link>
					<link><linkclipref>clipitem-{cid_a2}</linkclipref><mediatype>audio</mediatype><trackindex>2</trackindex><clipindex>{i+1}</clipindex><groupindex>1</groupindex></link>
				</clipitem>'''
